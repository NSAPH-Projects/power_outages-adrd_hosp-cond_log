...
# Author: Vivian  
# Date: 1/18/24
# Goal: generate visualization for figure 4
...
# read in results, filtering to Cutpoint 10% & lag 1
...
em_results_2 <- em_results %>% 
  ...
  mutate("Rate Ratio (95% CI)" = paste0(round(est, 2), " (", round(lci, 2), "-", round(uci, 2), ")")) %>% 
  group_by(r_geo_strata) %>% 
...
ggsave(plot = p,
       "/n/dominici_nsaph_l3/Lab/projects/power_outages-adrd_hosp-cond_log/figures/fig4_thresh10_lag1_main.png",
       dpi=300,
...
# Cutpoint 10%, Lag 1-2 (supplement) ----------------------------------------------------
...
ggsave(plot = p,
       "/n/dominici_nsaph_l3/Lab/projects/power_outages-adrd_hosp-cond_log/figures/fig4_thresh10_lag1_lag2.png",
       dpi=300,
       height=10, width=15, units="in")
